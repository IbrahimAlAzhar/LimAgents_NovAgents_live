"""
LimAgents + NovAgents – Multi-Agent Limitation & Novelty Generator
Three pipelines:
...
# App Setup
# ═══════════════════════════════════════════════════════════════
app = FastAPI(title="LimAgents + NovAgents API")
...
# ═══════════════════════════════════════════════════════════════
def retrieve_top3(openai_client: OpenAI,
...
async def pipeline_nov(api_key: str, pdf_bytes: bytes, model: str) -> AsyncGenerator[str, None]:
    try:
        yield event({"type": "status", "message": "Extracting text from PDF..."})
        raw = await asyncio.to_thread(extract_pdf_text, pdf_bytes)
        if len(raw.strip()) < 100:
        ...
        try:
            top3 = await asyncio.to_thread(retrieve_top3, client, query)
        except Exception as e:
...
async def endpoint_nov(
    ...
    pdf_bytes = await file.read()
    return StreamingResponse(
        pipeline_nov(api_key, pdf_bytes, model),
        media_type="text/event-stream",
        headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
    )
